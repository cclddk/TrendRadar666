...
def render_html_content(
    ...
    if report_data["stats"]:
        for stat in report_data["stats"]:
            ...
            color_style = "color:#059669;" if "随机" in word else ""
            html += f"""
                <div class="word-group">
                    <div class="word-header">
                        <div class="word-name" style="{color_style}">{word}</div>
    ...
    if report_data["failed_ids"]:
         html += f"<div style='color:red; font-size:12px;'>⚠️ 获取失败: {', '.join(report_data['failed_ids'])}</div>"
